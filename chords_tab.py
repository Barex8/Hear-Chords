...
class ChordsTab(ttk.Frame):
    ...
    def __init__(self,master):
        ...
        ttk.Label(self, text="Duración de cada acorde:").grid(column=0, row=2)

        ttk.Label(self, text="mín:", width=10).grid(column=0, row=3)
        self.dur_chord_min_input = ttk.Entry(self, width=3)
        self.dur_chord_min_input.grid(column=1, row=3)
        self.dur_chord_min_input.insert(0, "1")

        ttk.Label(self, text="máx:", width=10).grid(column=2, row=3)
        self.dur_chord_max_input = ttk.Entry(self, width=3)
        self.dur_chord_max_input.grid(column=3, row=3)
        self.dur_chord_max_input.insert(0, "1")


        ttk.Label(self, text="Duración entre acordes:").grid(column=0, row=4)

        ttk.Label(self, text="mín:", width=10).grid(column=0, row=5)
        self.dur_between_chord_min_input = ttk.Entry(self, width=3)
        self.dur_between_chord_min_input.grid(column=1, row=5)
        self.dur_between_chord_min_input.insert(0, "1")

        ttk.Label(self, text="máx:", width=10).grid(column=2, row=5)
        self.dur_between_chord_max_input = ttk.Entry(self, width=3)
        self.dur_between_chord_max_input.grid(column=3, row=5)
        self.dur_between_chord_max_input.insert(0, "1")
        ...
        self.sharp_chord = tkinter.BooleanVar(value=True)
        self.check_sharp = tkinter.Checkbutton(checkboxes_frame,text="¿Sostenidos?",variable= self.sharp_chord)
    ...
    def new_form(self):
        self.form["dur_chord_min"] = float( self.dur_chord_min_input.get())
        self.form["dur_chord_max"] = float( self.dur_chord_max_input.get())
        self.form["dur_between_chord_min"] = float (self.dur_between_chord_min_input.get())
        self.form["dur_between_chord_max"] = float (self.dur_between_chord_max_input.get())
        self.form["octave_min"] = int(self.octave_min_input.get())
        self.form["octave_max"] = int(self.octave_max_input.get())
        self.form["chord_played"] = []
...
#------ TEMA ACORDES ----------
    ...
    def play_sound(self,form = None):
        ...
        if not form["chord_played"]:
            random_chord_duration = random.uniform(form["dur_chord_min"],form["dur_chord_max"])

            notes = {}
            if self.sharp_chord.get():
                notes = self.notes
            else:
                for n in self.notes:
                    if not "#" in n:
            ...
            random_chord_name = random.choice(list(notes.keys()))
            random_octave = random.randint(form["octave_min"],form["octave_max"])
            chord_name = f"{random_chord_name}{random_octave}"
            print(f"chord: {chord_name}")

            random_chord_freq = self.change_octave(random_chord_name,random_octave)
            chord_wave = self.chord(root_freq=random_chord_freq,duration=random_chord_duration)
            sd.play(chord_wave,44100)
            form["chord_played"].append(Note(int(random_chord_duration * 1000), chord_wave, random_octave, chord_name))
            sd.sleep(int(random_chord_duration * 1000))
            sd.sleep(int(random.uniform(form["dur_between_chord_min"], form["dur_between_chord_max"]) * 1000))
            print("-------------------------")
        else:
            for note in form["chord_played"]:
